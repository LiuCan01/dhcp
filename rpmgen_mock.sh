...
/usr/bin/mock -r ${MOCK_CFG} --rootdir ${MOCK_CHROOT} --define "dist .el7.centos.es" --buildsrpm --spec ./SPECS/dhcp.spec --sources ./SOURCES/
/usr/bin/mock -r ${MOCK_CFG} --rootdir ${MOCK_CHROOT} --copyout /builddir/build/SRPMS/ ./SRPMS/
/usr/bin/mock -r ${MOCK_CFG} --rootdir ${MOCK_CHROOT} --define "dist .el7.centos.es" --rebuild ./SRPMS/dhcp-4.2.5-58.el7.centos.es.1.src.rpm
